...
use PhotoexpressoBundle\Entity\Address;
...
class AddressController extends Controller
{
    ...
    public function indexAction()
    {
        ...
        $addresses = $em->getRepository('PhotoexpressoBundle:Address')->findAll();
    ...
    {
        $address = new Address();
        $form = $this->createForm('PhotoexpressoBundle\Form\AddressType', $address);
        ...
        if ($form->isSubmitted() && $form->isValid()) {
            ...
            return $this->redirectToRoute('adress_show', array('id' => $address->getId()));
        }
    ...
    {
        $deleteForm = $this->createDeleteForm($address);
        $editForm = $this->createForm('PhotoexpressoBundle\Form\AddressType', $address);
        $editForm->handleRequest($request);

        if ($editForm->isSubmitted() && $editForm->isValid()) {
            $this->getDoctrine()->getManager()->flush();

            return $this->redirectToRoute('adress_edit', array('id' => $address->getId()));
        }
    ...
    private function createDeleteForm(Address $address)
    {
        return $this->createFormBuilder()
            ->setAction($this->generateUrl('adress_delete', array('id' => $address->getId())))
            ->setMethod('DELETE')
            ->getForm()
        ;
    }
